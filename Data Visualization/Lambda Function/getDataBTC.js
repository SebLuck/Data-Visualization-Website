...
// Endpoints name
const endpointName = "BitcoinEndpoint";
...
exports.handler = async (event) => {
    try {
        ...
        console.log("table 1: " + table1Data.Items[0].Price);
        ...
        let datesBTC = actualData.timestamp;
        let priceBTC = actualData.price;
        ...
        let startDate = sortDateBTC[sortDateBTC.length - 100];
        // let endDate = new Date(sortDateBTC[sortDateBTC.length - 1]);
        ...
        let last100Items = [];
        let startIndex = 0;
        if (sortPriceBTC.length >= 100){
            startIndex = sortPriceBTC.length - 100;
        }
        for(let i = startIndex; i < sortPriceBTC.length; i++){
            last100Items.push(sortPriceBTC[i]);
        }
        
        // console.log("Start: " + startingDate);
        // console.log("Target: "+last100Items.length);
        ...
        // console.log("Prediction: "+ responseData.predictions[0].mean);
        let bitcoinPrediction = [];
        // console.log(responseData.predictions[0].mean.length);
        for(let i = 0; i < responseData.predictions[0].mean.length; i++){
            bitcoinPrediction.push(responseData.predictions[0].mean[i]);
        }
